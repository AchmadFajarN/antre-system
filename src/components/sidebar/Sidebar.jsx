...
const Sidebar = ({ activeSidebar, setActiveSideBar }) => {
    ...
    return(
        <aside className={`fixed w-4/6 md:w-1/3 xl:w-2/8 ${ activeSidebar? '-translate-x-0': '-translate-x-full' } z-40 shadow-2xl xl:translate-x-0 transition-duration transition-transform bg-white h-screen`}>
            <div className="flex shadow-xl rounded-xl items-center pl-4 py-9 gap-4">
                <div className="size-20 rounded-full overflow-hidden">
                   <AdvancedImage className="w-full h-full object-center object-cover" cldImg={Cld.image(userData.url_photo )} />
                </div>
                <div>
                    <p className="md:text-xl font-semibold text-slate-800">{ userData.username }</p>
